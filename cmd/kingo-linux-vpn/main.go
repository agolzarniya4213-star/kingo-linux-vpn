...
func main() {
    ...
    if err != nil { slog.Error("Failed to init crypto layer", "error", err); os.Exit(1) }

    db, err := storage.NewSQLiteStorage(dbPath, crypto)
    if err != nil { slog.Error("Failed to init database", "error", err); os.Exit(1) }
    defer db.Close()
    ...
    go func() {
        if err := ipcServer.Start(ctx); err != nil { slog.Error("IPC server failed", "error", err); cancel() }
    }()
    ...
    for {
        select {
        case <-ctx.Done(): daemon.SdNotify(false, daemon.SdNotifyStopping); return
        case <-ticker.C: daemon.SdNotify(false, daemon.SdNotifyWatchdog)
        }
    }
}
